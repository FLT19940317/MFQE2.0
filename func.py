...
def transformer(batch, chan, flow, U , out_size, name='SpatialTransformer', **kwargs):
    ...
    def _interpolate(im, x, y, out_size):
        with tf.variable_scope('_interpolate'):
            ...
            #z = tf.cast(_repeat2(tf.range(0, 10), 10), 'float32')
            #z = _repeat(tf.range(0, 10), 10)
            ...
            # # motion = tf.zeros([height*width,3])
            # motion = tf.add(tf.pow(x,2), tf.pow(y,2))
            #
            # motion = tf.expand_dims(motion, 1)
            #
            # motion = tf.clip_by_value(motion-100.0, 0.0, 1.0)
            #
            # motion = tf.clip_by_value(motion*1000.0, 0.0, 1.0)
            ...
            #base = tf.zeros(out_height*out_width, dtype="int32")
...
# def batch_transformer(U, dx, dy, out_size, name='BatchSpatialTransformer'):
...
#         return transformer(input_repeated, dx, dy, out_size)
